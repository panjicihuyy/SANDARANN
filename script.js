...
/* =========================================================
   SANDARAN — EXTRA FEATURES SCRIPT
   File terpisah & aditif: hanya MENAMBAH event listener baru,
   tidak menyentuh/menimpa logika di script.js (keranjang, chatbot,
   checkout, dsb tetap berjalan seperti semula).
   ========================================================= */
(function(){
  "use strict";
  ...
  function init(){
    ...
    setupProgressBar();
    setupPromoTicker();
    setupAmbientCoffeeVisibility();
    setupBackToTop();
    setupCartMicroInteraction();
  }

  /* 1. SPLASH LOADER */
  function setupLoader(){
    var loader = document.getElementById("sdLoader");
    if (!loader) return;
    var hide = function(){ loader.classList.add("sd-loader--hide"); };
    // sembunyikan begitu halaman siap, dengan jeda singkat agar tidak "kedip"
    window.addEventListener("load", function(){ setTimeout(hide, 350); });
    // jaring pengaman: jangan sampai loader nyangkut kalau event load lambat
  ...
  /* 3. PROMO TICKER (bisa ditutup, ingat pilihan via localStorage) */
  function setupPromoTicker(){
    var promo = document.getElementById("sdPromo");
    var closeBtn = document.getElementById("sdPromoClose");
    if (!promo || !closeBtn) return;
    var KEY = "sandaran_promo_dismissed_v1";
    try{
      if (localStorage.getItem(KEY) === "1"){
        promo.classList.add("sd-promo--hide");
      }
  ...
  /* 4. SEMBUNYIKAN ANIMASI KOPI SAAT DI ATAS LIST PRODUK / BUNDLING */
  function setupAmbientCoffeeVisibility(){
    var cups = [
      document.getElementById("ambientCoffee1"),
      document.getElementById("ambientCoffee2")
    ].filter(Boolean);
    var targets = [
      document.getElementById("menu"),
      document.getElementById("bundling")
    ].filter(Boolean);
    if (!cups.length || !targets.length || !("IntersectionObserver" in window)){
      return;
    }
    var activeCount = 0;
    var toggle = function(){
      cups.forEach(function(cup){
        cup.classList.toggle("sd-hide", activeCount > 0);
      });
    };
    var observer = new IntersectionObserver(function(entries){
      entries.forEach(function(entry){
        if (entry.isIntersecting) activeCount++;
  ...
  function setupBackToTop(){
    var btn = document.getElementById("sdTopBtn");
    if (!btn) return;
    var hero = document.getElementById("hero");
    var onScroll = function(){
      var threshold = hero ? hero.offsetHeight * 0.6 : 400;
      var scrollTop = document.documentElement.scrollTop || document.body.scrollTop;
      btn.classList.toggle("sd-top-btn--visible", scrollTop > threshold);
    };
  ...
  /* 6. MICRO-INTERACTION: sparkle + bounce saat item masuk keranjang */
  function setupCartMicroInteraction(){
    var cartBtn = document.getElementById("cartToggle");
    var cartCount = document.getElementById("cartCount");
    if (!cartBtn || !cartCount || !("MutationObserver" in window)) return;

    var lastValue = cartCount.textContent;
    var observer = new MutationObserver(function(){
      var newValue = cartCount.textContent;
      if (newValue !== lastValue && Number(newValue) > Number(lastValue || 0)){
        burstSparkle(cartBtn);
        bumpCart(cartBtn);
      }
      lastValue = newValue;
    });
  ...
  function bumpCart(el){
    el.classList.remove("sd-bump");
    // reflow supaya animasi bisa diulang berturut-turut
    void el.offsetWidth;
    el.classList.add("sd-bump");
  }

  function burstSparkle(anchorEl){
    var rect = anchorEl.getBoundingClientRect();
    var wrap = document.createElement("div");
    wrap.className = "sd-sparkle-wrap";
    wrap.style.left = (rect.left + rect.width / 2) + "px";
    wrap.style.top = (rect.top + rect.height / 2) + "px";
    var icons = ["✨", "☕", "✨"];
    icons.forEach(function(icon, i){
      var span = document.createElement("span");
      span.className = "sd-sparkle";
      span.textContent = icon;
      var angle = (i - 1) * 28;
      var dist = 34 + Math.random() * 12;
      var rad = angle * (Math.PI / 180);
      span.style.setProperty("--sdx", Math.round(Math.sin(rad) * dist) + "px");
      span.style.setProperty("--sdy", Math.round(-Math.cos(rad) * dist - 20) + "px");
